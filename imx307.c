			      ...
			      const struct regval *regs)
{
	...
	for (i = 0; ret == 0 && regs[i].addr != REG_NULL; i++)
		if (unlikely(regs[i].addr == REG_DELAY))
			usleep_range(regs[i].val * 200, regs[i].val * 400);
		else
...
static int __imx307_power_on(struct imx307 *imx307)
{
	...
	if (!IS_ERR(imx307->csien_gpio))
       gpiod_set_value_cansleep(imx307->csien_gpio, 1);   
...
static void __imx307_power_off(struct imx307 *imx307)
{
	...
	if (!IS_ERR(imx307->csien_gpio))
		gpiod_set_value_cansleep(imx307->csien_gpio, 0);
			 ...
			 const struct i2c_device_id *id)
{
	...
	imx307->csien_gpio = devm_gpiod_get(dev, "csien", GPIOD_OUT_LOW);
	if (IS_ERR(imx307->csien_gpio))
