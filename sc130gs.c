...
//#define SC132GS_MODE_STREAMING		BIT(1)
...
disable_clk:
	...
	if (!IS_ERR_OR_NULL(sc132gs->pins_sleep))
		pinctrl_select_state(sc132gs->pinctrl, sc132gs->pins_sleep);
	return ret;
}
			...
			const struct i2c_device_id *id)
{
	...
	if (IS_ERR(sc132gs->xvclk)) {
		...
		sc132gs->reset_gpio = devm_gpiod_get(dev, "reset", GPIOD_OUT_HIGH);
	if (IS_ERR(sc132gs->reset_gpio))
		dev_warn(dev, "Failed to get reset-gpios\n");

	sc132gs->power_gpio = devm_gpiod_get(dev, "power", GPIOD_OUT_HIGH);
	if (IS_ERR(sc132gs->power_gpio))
		dev_warn(dev, "Failed to get power-gpios\n");

	sc132gs->vi_gpio = devm_gpiod_get(dev, "vi", GPIOD_OUT_HIGH);
	if (IS_ERR(sc132gs->vi_gpio))
