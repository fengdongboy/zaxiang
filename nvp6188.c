					       ...
					       u8 *ch_vfc)
{
	...
	while ((check_cnt++) < 50) {
		...
		if (ch_vfc[0] != 0xff || ch_vfc[1] != 0xff ||
			...
			ret = 0;
			if (ch == 3) {
				dev_err(&client->dev, "try check cnt %d",check_cnt);
				break;
			}
		} else {
	...
	if (ret) {
		...
		dev_err(&client->dev, "read vfc 0x%2x 0x%2x 0x%2x 0x%2x",
			   ...
			   struct v4l2_subdev_format *fmt)
{
	...
	dev_err(&client->dev, "%s: %x %dx%d vc %x\n",
				   ...
				   struct rkmodule_vc_fmt_info *inf)
{
	...
	dev_err(&nvp6188->client->dev, "nvp6188_get_vc_fmt_inf 0x%2x 0x%2x 0x%2x 0x%2x",
			ch_vfc[0], ch_vfc[1], ch_vfc[2], ch_vfc[3]);
